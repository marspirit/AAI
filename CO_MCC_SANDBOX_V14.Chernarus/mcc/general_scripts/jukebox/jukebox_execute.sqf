private ["_zone","_zonePos","_zoneX","_zoneY","_activate","_cond","_track","_trigger"];
...
jukeboxMusic = _this select 7;
		
_trigger = createTrigger ["EmptyDetector", _zonePos];
_trigger setpos _zonePos;
_trigger setTriggerArea [_zoneX, _zoneY, 0, true];
if (jukeboxMusic) then {_trigger setTriggerStatements ["this",format ["playMusic  '%1'",_track], ""];} 
	else {_trigger setSoundEffect["ICE_null", "", "",format ['%1',_track]];};
