private ["_vehicle", "_caller", "_coinVehicleType", "_coinRadius", "_coinFunds", "_pos", "_footer", "_html", "_break"];
_vehicle = _this select 0;
_caller = _this select 1;

_coinVehicleType = (_this select 3) select 0; 
switch ((_this select 3) select 1) do
	{
	case 0: 
		{
			_coinRadius =150;
...
if (((_this select 3) select 2)==0) then {_coinFunds=500} else
	{
	if (((_this select 3) select 2)<=10) then {_coinFunds=((_this select 3) select 2)*1000} else
		{_coinFunds=(((_this select 3) select 2)-9)*10000};
	};
_break = false; 
...
[0, {_this execvm "mcc\general_scripts\coin\coin_exec_srvr.sqf";}, [_pos,_coinRadius,_caller,_coinVehicleType,_coinFunds]] call CBA_fnc_globalExecute;
