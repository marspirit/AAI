...
MON_doParadrop = {
	...
	_units =[];
	_crew =[];
	// 100 should be safe for doint paradrop ? 
	if (_flyingheigh < 100) then {_flyingheigh = 100};
...
MON_landHely = {
	...
	_timeout = 60 + time;
	waituntil {!alive _heli || time > _timeout || ((velocity _heli select 2) <= 1 && ((position _heli) select 2) <= 1)};
	...
	if (((position _heli) select 2) > 2 && ((position _heli) select 2) < 30 && !surfaceiswater position _heli) then { 		
		_heli land "LAND";
		_timeout = 30 + time;
		waituntil {!alive _heli || time > _timeout || ((position _heli) select 2) > 30 || ((velocity _heli select 2) <= 1 && ((position _heli) select 2) <= 1)};	
	...
	//dogetout each soldier - gunners !RR
	{
		_x spawn MON_doGetOut;	
		sleep 0.5;
	}forEach crew _heli - [gunner _heli] - [driver _heli];	
...
MON_artillery_dofire = {
	if (!isserver) exitWith {}; 
	private ["_smoke1","_i","_area","_position","_maxcadence","_mincadence","_sleep","_rounds","_arti"];
	_area = 150;
	_maxcadence = 10;
	_mincadence = 5;
	_sleep = 0;
	...
	for [{_i=0}, {_i<_rounds}, {_i=_i+1}] do 
		...
		[[_smoke1], [format["bon_Shell_In_v0%1",[1,2,3,4,5,6,7] select round random 6],10]] call CBA_fnc_globalSay;
