...
waitUntil { !(isnil ("f_param_teleport"))  };
if (f_param_teleport == 1) then {teleportToTeam = true} else {teleportToTeam = false};
...
_unit = player;
_null =  _unit addaction ["Add/Remove Silencer", "mcc\general_scripts\silencer\silencer.sqf","",6,false,true,"","_target == _this"];
