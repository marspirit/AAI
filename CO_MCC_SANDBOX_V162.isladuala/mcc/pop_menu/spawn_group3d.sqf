	...
	while {MCC3DRuning} do
		...
		if (MCC3DRuning) then 
			...
			if (mcc_capture_state) then
				{
				 mcc_capture_var = mcc_capture_var + FORMAT ["['simpleSpawn', [%1, %2, '%3', '%4', '%5', %6, '%7', '%8']] call CBA_fnc_globalEvent;
									"
									,MCC3DValue select 0
									,MCC3DValue select 1
									,mcc_spawnname
					...
					{
					 mcc_safe = mcc_safe + FORMAT ["['simpleSpawn', [%1, %2, '%3', '%4', '%5', %6, '%7', '%8']] call CBA_fnc_globalEvent;
									mccFunctionDone = false; 
									waitUntil {mccFunctionDone};
									"
									,MCC3DValue select 0
									,MCC3DValue select 1
									,mcc_spawnname
					...
					["simpleSpawn", [MCC3DValue select 0, MCC3DValue select 1, mcc_spawnname, mcc_spawntype, mcc_spawnfaction, mcc_spawnwithcrew,mcc_unitInit,mcc_unitName]] call CBA_fnc_globalEvent;
					};
			};
		sleep 0.1;
		};
...
if (_3d == 2) then //Case we adding presets
	{	
	_tempText = ctrlText INITBOX;
	_presetText = (mccPresets select(lbCurSel PRESETS)) select 1; 
	ctrlSetText [INITBOX,format ["%1 %2",_tempText,_presetText]];
	};
