...
// ---------------------------------------------------------------------------------------------------------
	...
	// switch off ACE wounds system for AI
...
KRON_UPS_safedist = 200;

// how close unit has to be to target to generate a new one target or to enter stealth mode
KRON_UPS_closeenough = 300;

// how close AI units have to be to known enemy to share information, over this, will lose target
KRON_UPS_sharedist = 600;

//If enabled IA comunicating between them with radio defined sharedist distance, 0/2 (must be 2 to reinforcement works ok)
KRON_UPS_comradio = 2;
...
//% of chanse to use smoke by team members when someone wounded or killed in the group in %(default 15 & 40).
// set 0 to not use at all
R_USE_SMOKE_wounded = 13;
R_USE_SMOKE_killed = 35;

// knowsAbout 1.03 , 1.49 to add this enemy to target list (1-4) the higher number the less detect ability (original 0.5)
					...
					if (!isnull (gunner _arti) && canmove (gunner _arti)) then {
						...
						if (_fire) then {
										...
										if (alive _auxtarget && !(_auxtarget iskindof "Air") && (round([position _arti,_targetPos] call KRON_distancePosSqr)) <= _range) then {
											...
											{	
												if (!isnull _x && _side == side _x) then {																								
													if ((round([position _x,_targetPos] call KRON_distancePosSqr)) < (KRON_UPS_safedist/2)) exitwith {_target = objnull;};
												};										
											}foreach KRON_NPCs;																		
										};
									};
							...
							if (!isnull _target) then {
								...
								if (!isnil "_targetPos") then {
									[_arti,_targetPos,_rounds,_area,_maxcadence,_mincadence,_bullet] spawn MON_artillery_dofire;
								};
							};
						};
					};		
