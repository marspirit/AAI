...
//if (KRON_UPS_Debug>0) then {player sidechat format["%1: _cycle=%2 _currcycle=%3 _react=%4 _waiting=%5",_grpidx,_cycle,_currcycle,_react,_waiting]}; 
	...
	_newdamage=0; 
	{
		...
		if (_gothit && random 100<20) then {
			[_x,_target] spawn MON_throw_grenade;
			if (KRON_UPS_Debug>0) then {player sidechat format["%1: I'm Hit smoking!",_grpidx,_makenewtarget]}; 
					};
	} foreach _members;
