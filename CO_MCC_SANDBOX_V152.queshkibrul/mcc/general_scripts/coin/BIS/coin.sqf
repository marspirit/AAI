  ...
  */
	...
	while {!isnull _logic} do 
	  ...
	  debuglog format ["COIN_ACTION_ITER %1",[_logic,player]];
		if (isnil {player getvariable _BIS_COIN_idAction} && !isnull player) then 
		{
		  _player = player;
			...
			_action = player addaction [
				_description,
				"mcc\general_scripts\coin\BIS\coin_interface.sqf",
				_logic,
				1,
				false,
				false,
		...
		waituntil {Sleep 0.1, ((_player != player) && (!isnull player)) || ((_player distance _logic) > 300 + ((_logic getvariable 'BIS_COIN_areasize') select 0))};
	  ...
	  player setvariable [_BIS_COIN_idAction, nil];
		...
		waituntil {Sleep 3, (_player != player && !isnull player) || ((_player distance _logic) < (300 + _dis))};	//TODO:TESTME: if area of logic can grow much more than +1000 m in mission, direct area get should be used here (or action will be only in inner radius)
