private ["_ied","_side","_size","_targ","_count","_pos", "_enemy","_sbspeed","_sound","_armed"];
_ied  = _this select 0;
		...
		while {(alive _ied) && (_check) && (_t > 0)} do
			...
			for [{_x=0},{_x<_count},{_x=_x+1}] do
				...
				{if(_enemy isKindOf _x && _check) then
					{
					_ied setSpeedMode (_sbspeed select (round (random 1)));
					_ied domove (getpos _enemy);
					sleep 5 + random 10;
					if ((_ied distance _enemy) < 40) then {
							...
							while {alive _ied} do{
								...
								if ((_ied distance _enemy) <=20) exitwith { _check= false;};
								};
							};
					}
				} forEach _targ;
