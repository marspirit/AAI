			   ...
			   case "LGB":	//LGB
				...
				{ 
					...
					for [{_x=1},{_x<=_ammount},{_x=_x+1}] do 
						{
							_planepos = getpos _plane;
							_bomb = "Bo_Mk82" createvehicle [_planepos select 0,_planepos select 1,3000]; 	//make the bomb 
							_bomb setpos [_planepos select 0,(_planepos select 1)-4,(_planepos select 2) -2];
							_bomb setdir getdir _plane;
							[[_bomb], [format["bon_Shell_In_v0%1",[1,2,3,4,5,6,7] select round random 6],10]] call CBA_fnc_globalSay;
							sleep 0.3;
							_bomb setVelocity [((velocity vehicle _pilot) select 0)/3, ((velocity vehicle _pilot) select 1)/3,((velocity vehicle _pilot) select 2)*4];
							_bomb2 = "Bo_Mk82" createvehicle [_planepos select 0,_planepos select 1,3000]; 	//make the bomb 
							_bomb2 setpos [_planepos select 0,(_planepos select 1)+4,(_planepos select 2) -2];
							_bomb2 setdir getdir _plane;
							_bomb2 setVelocity [((velocity vehicle _pilot) select 0)/3, ((velocity vehicle _pilot) select 1)/3,((velocity vehicle _pilot) select 2)];
