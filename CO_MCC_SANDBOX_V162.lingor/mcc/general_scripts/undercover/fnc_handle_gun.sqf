...
switch (_type) do		
		{
			...
			reload _unit;
			_unit selectweapon "glock17_EP1";
			drawGunIsRuning = false; 
			_unit removeaction _index;
			_null = _unit addaction ["Holster Weapon", "mcc\general_scripts\undercover\fnc_handle_gun.sqf",1,6,false,true,"","_target == _this"];
			};
			...
			drawGunIsRuning = false; 
			_unit removeaction _index;
			_null = _unit addaction ["Draw Weapon", "mcc\general_scripts\undercover\fnc_handle_gun.sqf",0,6,false,true,"","_target == _this"];
			};
		};
