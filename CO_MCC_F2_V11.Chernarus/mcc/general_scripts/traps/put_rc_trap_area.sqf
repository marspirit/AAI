...
_eib_marker = createMarkerLocal ["area",_this];
_eib_marker setMarkerShapeLocal "ELLIPSE";
_eib_marker setMarkerSizeLocal [100, 100];
...
for "_x" from 1 to trapsnumber do
    {        
	...
	_tripwire_point_a = [_validpos select 0, _validpos select 1, _z];
	_tripwire_point_b = [(_validpos select 0) + ((sin direction _rc) * 20),(_validpos select 1) + ((cos direction _rc) * 20),_z];
	...
	_ied = [_rc,iedside,trapvolume,30,10,_trigger] execVM "mcc\general_scripts\traps\IED_rc.sqf";
	sleep 0.1;
