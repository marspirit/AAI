...
if (_type == 1) then //Add weapon + mags
	...
	if (gearDialogClassIndex == 9) then
		{
		tempBox addMagazineCargo [_class,1];
		} else
		{
...
if (_type == 4) then //Generate
	...
	closedialog 0; 
	hint "click on map"; 
	onMapSingleClick "_nul=[""GuerillaCacheBox_EP1"",_pos] call MCC_3D_PLACER;closeDialog 0;onMapSingleClick """";";	
	deletevehicle Object3D;
	Object3D = "GuerillaCacheBox_EP1" createvehicle [0,0,0];	
	...
	while {MCC3DRuning} do
		...
		if (MCC3DRuning) then 
			{
			 mcc_safe = mcc_safe + FORMAT ["['boxGenerator', [%1, %2, %3, %4]] call CBA_fnc_globalEvent;sleep 1;"								 
						,MCC3DValue select 0
						,MCC3DValue select 1
						,tempBoxWeapons
						,tempBoxMagazine
						];	
			["boxGenerator", [MCC3DValue select 0, MCC3DValue select 1, tempBoxWeapons, tempBoxMagazine]] call CBA_fnc_globalEvent;
			};
		sleep 0.1;
		};
	};
